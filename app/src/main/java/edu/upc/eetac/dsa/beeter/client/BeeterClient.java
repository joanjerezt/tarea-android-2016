...
public class BeeterClient {
    private final static String BASE_URI = "http://192.168.1.101:8080/beeter";
    ...
    public String createSting(String subject, String content) throws BeeterClientException {
        String loginUri = getLink(root.getLinks(), "create-sting").getUri().toString();
        WebTarget target = client.target(loginUri);
        Form form = new Form();
        form.param("subject", subject);
        form.param("content", content);
        String json = target.request().header("X-Auth-Token", authToken).post(Entity.entity(form, MediaType.APPLICATION_FORM_URLENCODED_TYPE), String.class);
        //if (response.getStatus() == Response.Status.OK.getStatusCode())
        //    return response.readEntity(String.class);
        //else
        //    throw new BeeterClientException(response.readEntity(String.class));
